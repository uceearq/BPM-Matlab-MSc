...
function n_profile = calcRI_7bundle(X,Y,n_background,taper)
X = X.*taper;
Y = Y.*taper;

% Define parameters
core_radius = 8.2 / 2 * 1e-6; % Core radius in micrometers
cladding_radius = 80 / 2 * 1e-6; % Cladding radius in micrometers
...
% figure;
% imagesc(X(1:end,1), X(1:end,1), n_profile);
% axis equal;
% colorbar;
